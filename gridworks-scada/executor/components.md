...
`MakeModel` (`enums/make_model.py`, 50+ values, a sema enum) is the
make/model of a device. The Cac carries it; an actor reaches it as
`self.node.component.cac.MakeModel`. `CACS_BY_MAKE_MODEL` is a static
`MakeModel → UUID` dict every known make/model must appear in.

**Sim already lives here.** Six legacy `GRIDWORKS__SIM*` make/models exist
(`SIMPM1`, `SIMBOOL30AMPRELAY`, `SIMTSNAP1`, `SIMMULTITEMP`,
`SIMTOTALIZER`, `SIMDOUBLE16PINI2CRELAY`) — i.e. the layout already has a
way to say "this device is simulated" at the make/model grain. This is
the seam the simulated-actors design builds on (sim declared at the device
boundary, legible in the layout), minus the legacy driver-class
indirection that sat beneath it (see `drivers/power_meter/`, the
"Russian dolls" anti-pattern).
...
2. **3 of 18 device types are DB-backed (`*.cac.gt`) — by design.** A device
   ...
   their identity carried by `MakeModel`. (The open question is narrower: is
...
4. **Two ways to say "simulated."** Legacy: a `GRIDWORKS__SIM*` MakeModel
   on a generic component (sim is opaque in the component). Newer: a
   dedicated `sim.*.component.gt` TypeName with
   `SimulatesTypeName`/`SimulatesVersion` (only `sim.pico.tank` so far).
   No unified pattern yet — the simulated-actors design picks one.
