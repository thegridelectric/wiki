...
- **gridworks-scada** — [`designs/channel-config-overhaul.md`](gridworks-scada/designs/channel-config-overhaul.md) (settle channel.config + thin board-resident-component shape while component versions are unpublished — channel identity from TelemetryName, routing from DerivedChannel.InputChannelNames; Linear OPS-427)
...
- **gridworks-scada** — [`designs/hardware-layout-pass-one.md`](gridworks-scada/designs/hardware-layout-pass-one.md) (first critical pass on the hardware-layout/components model: drop UUID cac_ids → `gw1.device.type` enum + `DeviceType`, simplify components, restructure `layout_gen`; shared dependency of simulated-test-environment + spruce-unlimbo Chunk B; Linear OPS-407)
...
- **gridworks-scada** — [`designs/scada-health-diagnostics.md`](gridworks-scada/designs/scada-health-diagnostics.md) (fast scada-down detection via live ally.inactive + persist the liveness signal set; ex-OPS-386 #3; Linear OPS-317)
