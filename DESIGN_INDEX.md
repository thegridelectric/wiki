...
- **rmqbot** — [`designs/broker-topology-canonical-in-gwbase.md`](rmqbot/designs/broker-topology-canonical-in-gwbase.md) (vhosts/exchanges/queues/bindings declared as code in gwbase → a CLI generates the definitions JSON (vhost a parameter, default d1__1) + drift test; retires hand-edited rabbit_definitions.json + fixes the dev conf's vhost; lands with the 4.x upgrade; Linear OPS-425)
