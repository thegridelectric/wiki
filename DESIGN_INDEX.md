...
- (cross-cutting) [`designs/ltn-brokered-app-comms.md`](designs/ltn-brokered-app-comms.md) (web→LTN→SCADA application comms — mode/params/preferences brokered through the LTN, in Auto, SLA-preserving; the admin/direct-relay half is separate (OPS-529, mtls-dependent); Linear OPS-408)
...
- **gridworks-admin** — [`designs/local-admin-session.md`](gridworks-admin/designs/local-admin-session.md) (the admin session and the admin process the named humans stand behind, on the Pi's local broker: take-control naming the human, heartbeat.a with a session id, holder-changed on takeover, one MQTT client id; no envelope dependency; Linear OPS-529)
- **gridworks-admin** — [`designs/admin-on-the-fabric.md`](gridworks-admin/designs/admin-on-the-fabric.md) (the admin session moved onto the prod broker as `hw1.admin` over the addressed envelope, humans with personal certs behind the admin process; depends on OPS-428 and OPS-420; Linear OPS-429)
...
- **gridworks-ltn** — [`designs/house-params-and-working-store.md`](gridworks-ltn/designs/house-params-and-working-store.md) (house thermal parameters as a sema JSON file beside the layout, a bounded rebuildable SQLite working store per LTN, the surface a fitted parameter set arrives on; operator override to grill; EDD; Draft, OPS-531)
...
- **gridworks-admin** — [`designs/scada-owned-admin-hold.md`](gridworks-admin/designs/scada-owned-admin-hold.md) (scada-owned admin hold that survives the panel dropping + reports remaining time; Linear OPS-194)
...
- **gridworks-scada** — [`designs/spruce-settled/primary.md`](gridworks-scada/designs/spruce-settled/primary.md) (the work after the spruce launch: every command node's machine state a journal channel by one rule, which changes report asynchronously; folder name proposed 2026-09-10, no Linear issue yet)
...
- **sema** — [`designs/launch-sema/primary.md`](sema/designs/launch-sema/primary.md) (what "Sema is launched" means before the 2026–27 heating season: schema URLs resolve at schemas.electricity.works, consumer conformance where word status decides severity so daily sema change never reddens a consumer by itself, the snapshot drift check (folded in from OPS-536), and a front door for strangers; hub; Draft, OPS-538)
...
- **sema** — [`designs/hash-pin-ignores-examples.md`](sema/designs/hash-pin-ignores-examples.md) (published-hash pin hashes validating content, not bytes, so a superseded version can gain its `examples:` block without a re-pin; Draft, OPS-528)
