...
- **grid-node-registry** — [`designs/stand-up-grid-node-registry/primary.md`](grid-node-registry/designs/stand-up-grid-node-registry/primary.md) (stand up the GNode registry FIS consults: dev Postgres + alembic schema → invariants → lifecycle → write/read surfaces → populate + deploy on EC2; hub + test-harness / populate-and-deploy spokes; 2026-summer; Linear OPS-419)
...
- **rmqbot** — [`designs/broker-topology-canonical-in-gwbase.md`](rmqbot/designs/broker-topology-canonical-in-gwbase.md) (vhosts/exchanges/queues/bindings declared as code in gwbase, generated never hand-edited; dev half built (gen_definitions.py + drift guards); remaining: prod + analytics targets, infra deploy copy generated, legacy-file/deploy-kit cleanup; lands with the 4.x upgrade; Linear OPS-425)
- **rmqbot** — [`designs/prod-4x-upgrade.md`](rmqbot/designs/prod-4x-upgrade.md)
- **rmqbot** — [`designs/prod-tls-fix.md`](rmqbot/designs/prod-tls-fix.md) (revive the expired PKI + encryption-only TLS on the prod broker, on 3.9.13 decoupled from 4.x: 10-y CA-2026, one 2-y broker cert, verify_peer + optional client certs, parameterized conf, smqPublic rotation, EDD harness; notch 1 of the ratchet to mTLS; Accepted; Linear OPS-423)
