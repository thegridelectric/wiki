...
|---|---|---|---|
| **Command surface** | The declared set of commands one party offers to exactly one kind of counterparty, with the owner enforcing authority and every command answered by an ack or a nack with a reason. The pattern and the rules: [`command-surface.md`](command-surface.md). | [`gw.command.interface`](../sema/definitions/types/gw.command.interface/000.yaml), [`gw.dispatch.ack`](../sema/definitions/types/gw.dispatch.ack/000.yaml), [`gw.dispatch.nack`](../sema/definitions/types/gw.dispatch.nack/000.yaml) | — (concept; each surface lives in its owner's repo) |
...
| **GNode** (Grid Node) | The fundamental identity object in the GridWorks ecosystem. Every physical location, metered unit, device, service, or actor is represented as a GNode. Has two classifications: **`BaseClass`** (universal structural ontology, an enum — same values across every organization for interop) and **`GNodeClass`** (free-form string, interpreted in the registering org's namespace — captures functional/role-based meaning that doesn't generalize across orgs). The asymmetry is by design: enum where the world must agree (cross-org topology and market coordination), free-form string where each org names its own roles. **`GNodeId` is the immutable identity**; `Alias` and `GNodeClass` evolve over the life of the GNode — e.g., a node first registered as `ConnectivityNode` can become a `MarketMaker` once a constraint emerges and local clearing is needed. The GridWorks-specific functional taxonomy lives in [`gw.g.node.class`](../sema/definitions/enums/gw.g.node.class/000.yaml). See **GNodeAlias**. | [`g.node.gt`](../sema/definitions/types/g.node.gt/004.yaml), [`base.g.node.class`](../sema/definitions/enums/base.g.node.class/000.yaml), [`gw.g.node.class`](../sema/definitions/enums/gw.g.node.class/000.yaml) | — (concept; per-org GNode registries vary) |
| **GNodeAlias** | The mutable hierarchical name of a [GNode](#gridworks-concepts) — a dotted token sequence in the [`left.right.dot`](../sema/definitions/formats/left.right.dot.yaml) grammar (e.g., `d1.isone.me.versant.keene.beech`). Encodes the GNode's position in the collaborative grid coordinate system. **Aliases are explicitly mutable** as shared comprehension of the grid deepens, new constraints emerge, or the physical grid itself changes — e.g., a node may be re-parented under a newly recognized constraint point, or renamed when a previously implicit boundary becomes a participating GNode. Alias transitions are captured via `PrevAlias` on the `g.node.gt` snapshot; the durable identity is `GNodeId`. Suffix semantics are constrained by axiom: `.ta` for TerminalAsset, `.scada` for Scada. The bigger vision: as entities join EEMs and other GridWorks coordination, they end up **co-creating a collaborative map of the low-voltage electric grid** by agreeing on shared GNodeAliases — the alias namespace IS the shared map. | [`g.node.gt`](../sema/definitions/types/g.node.gt/004.yaml) (fields `Alias`, `PrevAlias`, `GNodeId`), [`left.right.dot`](../sema/definitions/formats/left.right.dot.yaml) | — (concept; per-org alias registries) |
